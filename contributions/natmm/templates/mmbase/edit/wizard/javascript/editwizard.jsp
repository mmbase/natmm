 ...
 *
 * @since    MMBase-1.6
 * @version  $Id: editwizard.jsp,v 1.1 2006-03-05 21:46:43 henk Exp $
 * @author   Kars Veling
...
function doStartUpload(el) {
    ...
    // hh added focus on window
    var newwin = window.open(href,null,"width=300,height=300,status=yes,toolbar=no,titlebar=no,scrollbars=no,resizable=no,menubar=no,top=100,left=100");
    newwin.focus();    
    return false;
}
...
function setFocusOnFirstInput() {
    ...
    for (var i=0; i < form.elements.length; i++) {
        ...
        if (hidden != "hidden") {
            // hh: added from override.js because the setFocusOnFirstInput there, was never called
