...
public class UpdateUnusedElements implements Runnable {
   ...
   public ArrayList getUnusedItemsForRubriek(String rubriekNumber) {
      ArrayList allUnusedItems = new ArrayList();

      NodeList nlElements = cloud.getList(rubriekNumber, "rubriek,creatierubriek,contentelement", "contentelement.number", null, null, null, null,
            false);
      for (int k = 0; k < nlElements.size(); k++) {
         ...
         if (ch.usedInItems(node) == null) {
            ...
            allUnusedItems.add(node);
         }
      }
      log.debug("found " + allUnusedItems.size() + " unused items for rubriek " + rubriekNumber);
      return allUnusedItems;
   }
   ...
   /*
    * Deletes all unused objects of nodeType i.e. objects that do not have a
    * parent
    */
   ...
   /*
    * Checks whether each object has at most 3 archived copies, and deletes all
    * other copies
    */
      ...
      NodeList nl = cloud.getList("","archief",
         ...
         for (Iterator it = nl.iterator(); it.hasNext();) {
            ...
            if (set.contains(node.getStringValue("archief.original_node"))) {
               ...
               cloud.getNode(node.getStringValue("archief.number")).delete();
               copiesDeleted++;
            } else {
   ...
   private Thread getKicker() {
      ...
      if (kicker.getName().indexOf("UpdateUsedElementsThread") == -1) {
         kicker.setName("UpdateUsedElementsThread / " + ServerUtil.getDateTimeString());
